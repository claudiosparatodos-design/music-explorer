...
Albums are shown newest first. Every failure mode (no results, API error, no network) produces a readable message instead of a blank screen.
...
Request parameters: `media=music`, `entity=song`, `limit=200` (the API maximum).
...
**6. Small display touches.** The year is sliced off the ISO date. `artworkUrl100` is a 100×100 thumbnail, but the CDN serves other sizes at the same path, so the URL's `100x100` is swapped for `300x300` to get a sharper cover.
...
- **No pagination.** One request with `limit=200`, the API maximum. For a prolific artist that truncates the catalogue, and because the API sorts by relevance rather than date, the cut-off is arbitrary. Proper handling would page through with an offset and show a "load more" control.
